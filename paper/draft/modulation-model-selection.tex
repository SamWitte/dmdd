...
\abstract{
It has recently been demonstrated that, in the event of a putative signal in future dark matter direct detection experiments, properly identifying the underlying dark matter-nuclei interaction promises to be a challenging task. Given the most optimistic expectations for the number counts of recoil events in the \sjwrm{coming Generation 2 (G2)} \sjw{next-generation (\ie Generation 2)} experiments, \sjwrm{even the} interaction\sjw{s} \sjwrm{theories} that produce distinct features in the recoil energy spectra will be \sjwrm{possible to mutually distinguish}\sjw{distinguishable} only if a strong signal is observed simultaneously on a variety of complementary targets. However, there is a wide range of viable theories that give rise to virtually identical energy spectra, and may only differ \sjwrm{by the power of the dark-matter velocity dependence}\sjw{by the interaction's dependence on the dark matter velocity}. In this work, we investigate how degeneracy between such competing models may be broken by considering \sjwrm{time information on when the events are recorded in data analysis}\sjw{the time dependence of nuclear recoils (\ie the annual modulation of the rate)}. For this purpose, we \sjwrm{create a suite of recoil data simulations (under a variety of interaction models, including the appropriate levels of Poisson noise, and assuming G2 and larger exposures)}\sjw{simulate dark matter events for a variety of interactions and experimental exposures,} and perform a Bayesian model-selection analysis on all simulated data sets, evaluating the chance of correct identification of the input model for a given experimental setup. We find that including information on the annual modulation of the rate may significantly enhance the ability of a single target to distinguish dark matter models with nearly degenerate recoil spectra, but only with exposures beyond the expectations of G2 experiments.}
