...
\abstract{
It has recently been demostrated that, in the event of a putative signal in future dark matter direct detection experiments, properly identifying the underlying dark matter-nuclei interaction promises to be a challenging task. Given most optimistic expectations for the number counts of recoil events in the coming Generation 2 (G2) experiments, even the interaction theories that produce distinct fetures in the recoil energy spectra will be possible to mutually distinguish only if a strong signal is observed simultaneously on a variety of complementary targets. However, there is a wide range of viable theories that give rise to virtually identical energy spectra, and may only differ by the power of the dark-matter velocity dependence. In this work, we investigate how degeneracy between such compeeting models may be broken by considering time information on when the events are recorded in data analysis. For this purpose, we create a suite of recoil data simulations (under a variety of interaction models, including the appropriate levels of Poisson noise, and assuming G2 and larger exposures) and perform a Bayesian model-selection analysis on all simualted data sets, evaluating the chance of correct identification of the input model, for a given experimental setup. We find that including information on the annual modulation of the rate may significantly enhance the ability of a single target to distinguish dark matter models with nearly degenerate recoil spectra, but only with exposures beyond the expectations of G2 experiments.}
...
\begin{eqnarray} \label{eq:photon-DM-ops}
\cO_{\rm \chi, MD}^\mu & = & \frac{g^{\rm MD}}{q} \bar \chi i \sigma^{\mu \nu}  \chi ,\\
\cO_{\rm \chi, ED}^\mu & = & \frac{g^{\rm ED}}{q} \bar \chi i \sigma^{\mu \nu} \gamma_5 \chi.
\end{eqnarray}
...
\\ \hline
Magnetic Dipole (heavy) & $\frac g{ M^2} \bar{\chi} \sigma^{\mu \nu} \chi  \, q_\nu \cJ_\mu $ & $\frac{\vec q^{\,4}}{\Lambda^4}+ \frac{\vec{q}^2 v_\perp^2 }{\Lambda^2},\vec q^{\,4}/\Lambda^4$  
\\ \hline
Electric Dipole (heavy) &$ \frac g{M^2} \bar{\chi} \sigma^{\mu \nu} \gamma_5 \chi \, q_\nu \cJ_\mu $ & $\vec{q}^2 /\Lambda^2 $
\\ 
\hline 
Magnetic Dipole (light) & $\frac{g}{q^2} \bar{\chi} \sigma^{\mu \nu} \chi F_{\mu\nu} $ & $1+ \frac{v_\perp^2 m_N^2}{\vec{q}^2 }$, 1  
\\ \hline
Electric Dipole (light) & $\frac{g}{q^2} \bar{\chi} \sigma^{\mu \nu} \gamma_5 \chi F_{\mu\nu} $ & $m_N^2/\vec{q}^2 $
\\ \hline \hline
\end{tabular}
